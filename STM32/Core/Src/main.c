...
int main(void)
{
  ...
  MX_DMA_Init();
  MX_FMC_Init();
  /* SPI4 is intentionally not initialized: PE2/PE6 share the Xilinx QSPI
     configuration bus and must be high-Z while Artix-7 is Master SPI. */
  MX_DMA2D_Init();
  MX_LTDC_Init();
  /* USER CODE BEGIN 2 */
  if (LCD_Status_Init() != HAL_OK)
  {
    Error_Handler();
  }
...
void MPU_Config(void)
{
  ...
  */
  ...
  /* FMC Bank1 (NE1): non-cacheable, non-bufferable 16-bit stream port. */
  MPU_InitStruct.Enable = MPU_REGION_ENABLE;
  ...
  /* Match the verified DSO firmware: keep D2 SRAM1 non-cacheable for
     ETH descriptors and the LwIP heap at 0x30004000. */
  MPU_InitStruct.Enable = MPU_REGION_ENABLE;
...
void Error_Handler(void)
{
  ...
  fault_dbg.reason = 10U;
  fault_dbg.count++;
  fault_dbg.ipsr = __get_IPSR();
  fault_dbg.cfsr = SCB->CFSR;
  fault_dbg.hfsr = SCB->HFSR;
  fault_dbg.dfsr = SCB->DFSR;
  fault_dbg.afsr = SCB->AFSR;
  fault_dbg.mmfar = SCB->MMFAR;
  fault_dbg.bfar = SCB->BFAR;
...
void assert_failed(uint8_t *file, uint32_t line)
{
  ...
  fault_dbg.reason = 11U;
  fault_dbg.count++;
  fault_dbg.assert_file = (const char *)file;
  fault_dbg.assert_line = line;
  /* USER CODE END 6 */
}
